package pageObjects.pages;

import org.openqa.selenium.Keys;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindBy;
import org.testng.Assert;
import pageObjects.initializePageObjects.PageFactoryInitializer;
import ru.yandex.qatools.allure.annotations.Step;

import java.util.List;

public class HistoryTrades extends PageFactoryInitializer {

    @FindBy(xpath = "//a//b[contains(text(),'Trades')]")
    private WebElement ordersLink;

    @FindBy(xpath = "//div[div[contains(text(),'Instrument')]]//input")
    private WebElement instrumentFilter;

    @FindBy(xpath = "//div[div[contains(text(),'Instrument')]]//div[@class='Select-placeholder']")
    private WebElement instrumentValue;

    @FindBy(xpath = "//div[label[contains(text(),'From')]]//input")
    private WebElement fromDatePicker;

    @FindBy(xpath = "//div[label[contains(text(),'To')]]//input")
    private WebElement toDatePicker;

    @FindBy(xpath = "//div[contains(@class,'HistoryPage-containers-Trades')]//span[contains(text(),'Today')]")
    private WebElement todaySpan;

    @FindBy(xpath = "//div[contains(@class,'HistoryPage-containers-Trades')]//span[contains(text(),'Yesterday')]")
    private WebElement yesterdaySpan;

    @FindBy(xpath = "//div[contains(@class,'HistoryPage-containers-Trades')]//span[contains(text(),'Last week')]")
    private WebElement lastWeekSpan;

    @FindBy(xpath = "//div[contains(@class,'HistoryPage-containers-Trades')]//span[contains(text(),'Last month')]")
    private WebElement lastMonthSpan;

    @FindBy(xpath = "//div[contains(@class,'containers-Trades-___styles__trade___2Ip_H')]")
    private List<WebElement> tradesList;

    @FindBy(xpath = "//div[contains(@class,'styles__trade___2Ip_H')]//div[contains(@class,'styles__colored___24KfJ')]")
    private List<WebElement> idList;

    @FindBy(xpath = "//div[div[contains(text(),'Side')]]//span[contains(text(),'All')]")
    private WebElement allSidesSpan;

    @FindBy(xpath = "//div[contains(@class,'Switchers')]//span[text()='Buy']")
    private WebElement buySideSpan;

    @FindBy(xpath = " //div[contains(@class,'Switchers')]//span[text()='Sell']")
    private WebElement sellSideSpan;

    @FindBy(xpath = "//button[contains(text(),'Reset all filters')]")
    private WebElement resetAllFiltersButton;

    @FindBy(xpath = "//input[@name='id']")
    private WebElement notificationId;

    @Step("Click on trades link ")
    public HistoryTrades clickTradesLink() {
        utils.FluentWaiting.waitUntillElementToBeClickable(30, 500, ordersLink);
        ordersLink.click();
        return this;
    }

    @Step("Enter date From {0}")
    public HistoryTrades enterDateFrom(String date) {
        fromDatePicker.sendKeys(date);
        return this;
    }

    @Step("Clear field datepicker From field")
    public HistoryTrades clearDateFrom() {
        sleep(2000);
        scrollIntoView(fromDatePicker);
        this.clearInput(fromDatePicker);
        return this;
    }

    @Step("Clear field datepicker To field")
    public HistoryTrades clearDateTo() {
        sleep(2000);
        scrollIntoView(toDatePicker);
        this.clearInput(toDatePicker);
        return this;
    }

    @Step("Enter date To {0}")
    public HistoryTrades enterDateTo(String date) {
        toDatePicker.sendKeys(date);
        return this;
    }

    @Step("Click on Today span")
    public HistoryTrades clickToday() {
        sleep(2000);
        todaySpan.click();
        return this;
    }

    @Step("Click on Yesterday span")
    public HistoryTrades clickYesterday() {
        sleep(2000);
        yesterdaySpan.click();
        return this;
    }

    @Step("Click on Last week span")
    public HistoryTrades clickLastWeek() {
        sleep(2000);
        lastWeekSpan.click();
        return this;
    }

    @Step("Click on Last month span")
    public HistoryTrades clickLastMonth() {
        sleep(2000);
        lastMonthSpan.click();
        return this;
    }

    @Step("Click on all side span")
    public HistoryTrades clickAllSides() {
        sleep(2000);
        allSidesSpan.click();
        return this;
    }

    @Step("Click on buy side span")
    public HistoryTrades clickBuySide() {
        sleep(2000);
        buySideSpan.click();
        return this;
    }

    @Step("Click on sell side span")
    public HistoryTrades clickSellSide() {
        sleep(2000);
        sellSideSpan.click();
        return this;
    }


    public int getAllTradesNumber() {
        int size;
        for (size = 0; size < tradesList.size(); ++size)
            scrollIntoView(tradesList.get(size));
        scrollIntoView(tradesList.get(0));
        return size;
    }

    public int getTradesNumber() {
        return tradesList.size();
    }

    @Step("verify if trades are updated . size before {0}, size after {1}")
    public HistoryTrades verifyTradesAreUpdated(int listSizeBefore, int listSizeAfter) {
        sleep(3000);
        if (tradesList.size() > 0)
            Assert.assertNotEquals(listSizeBefore, listSizeAfter);
        else
            Assert.assertEquals(tradesList.size(), 0);
        return this;
    }




    public HistoryTrades verifyTradesSize(int size) {
        Assert.assertEquals(tradesList.size(), size);
        return this;
    }


    @Step("verify if date on datepicker <<from>> is right {0}")
    public HistoryTrades verifyDatePicker(String date) {
        Assert.assertEquals(fromDatePicker.getAttribute("value"), date);
        return this;
    }

    @Step("verify if no available trades")
    public HistoryTrades noTradesAreAvailable() {
        Assert.assertEquals(tradesList.size(), 0);
        return this;
    }

    @Step("verify if there are available trades")
    public HistoryTrades tradesAreAvailable() {
        sleep(5000);
        Assert.assertNotEquals(tradesList.size(), 0);
        return this;
    }

    @Step("Set instrument to {0}")
    public HistoryTrades setInstrument(String instrument) {
        scrollIntoView(instrumentFilter);
        instrumentFilter.sendKeys(instrument + Keys.ENTER);
        return this;
    }

    @Step("Enter the id {0}")
    public HistoryTrades enterId(String idString) {
        scrollIntoView(notificationId);
        notificationId.sendKeys(idString);
        return this;
    }


    @Step("Clear notification id field")
    public HistoryTrades clearId() {
        this.clearInput(notificationId);
        return this;
    }

    @Step("Click on <<Reset all filters>> button")
    public HistoryTrades clickOnResetAllFiltersButton() {
        sleep(2000);
        if(resetAllFiltersButton.isEnabled()) {
            resetAllFiltersButton.click();
        }
        return this;
    }

    @Step("Verify datepicker, instrument and status")
    public HistoryTrades verifyValuesWhenFilterIsReset(String date) {
        Assert.assertEquals(fromDatePicker.getAttribute("value"), date);
        Assert.assertEquals(instrumentValue.getText(), "All");
        return this;
    }

    @Step("get first id of trades list")
    public String getId() {
        return idList.get(0).getText();
    }


}
